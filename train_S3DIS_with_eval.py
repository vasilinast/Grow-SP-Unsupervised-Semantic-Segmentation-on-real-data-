...
def main(args, logger):
    ...
    # Define full paths
    eval_df_path = os.path.join(args.save_path, "eval_scores.txt")
    ...
    for epoch in range(1, args.max_epoch[0] + 1):
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                o_Acc, m_Acc, s, silhoutte, wcss = eval(epoch, args, test_areas)
                logger.info('Epoch: {:02d}, oAcc {:.2f}  mAcc {:.2f} IoUs'.format(epoch, o_Acc, m_Acc) + s)

                # Append to the main DataFrame
                new_row = pd.DataFrame([o_Acc, m_Acc, s, silhoutte, wcss], index=[epoch])
                eval_df = pd.concat([eval_df, new_row])
                eval_df.to_csv(eval_df_path, sep="\t", index=True)
