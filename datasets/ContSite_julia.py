...
class ConstSite(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Find the region with the highest point count
        unique_regions, region_counts = np.unique(region, return_counts=True)
        max_count_region = unique_regions[np.argmax(region_counts)]

        # Create a mask to filter out the region with the highest point count
        mask = region != max_count_region

        # Apply the mask to coords, colors, and labels
        coords = coords[mask]
        colors = colors[mask]
        labels = labels[mask]
        region = region[mask]
        ...
        # region_file = self.args.sp_path + '/' +self.name[index] + '_superpoint.npy'
        # region = np.load(region_file)
        # print("region in getitem")
        # print(region.shape)
        # reg_un = np.unique(region, return_counts=True)
        # print(reg_un)
        # print(reg_un[0].shape)

        '''Clip if Scene includes much Points'''
        #if clip_inds is not None:
        #     region = region[clip_inds]
        region = region[unique_map]
        print("region after unique_map in getitem")
        print(unique_map)
        print(region.shape)
        reg_un = np.unique(region, return_counts=True)
        print(reg_un)
